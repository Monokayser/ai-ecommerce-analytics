...
def geographic_chart(frame: pd.DataFrame, context: str = "") -> go.Figure:
    ...
    if "Country" in frame and metric in frame:
        ...
        figure = px.choropleth(grouped, locations="Country", locationmode="country names", color=metric, color_continuous_scale="Teal", title=f"{metric} by Country")
        if grouped["Country"].nunique() > 1:
            return apply_theme(figure, source_context=context)
...
def distribution_chart(frame: pd.DataFrame, metric: str = "Sales", context: str = "") -> go.Figure:
    ...
    figure = px.histogram(frame, x=metric, color=color, marginal="box", nbins=40, title=f"Distribution of {metric}")
    return apply_theme(figure, source_context=context)
...
def hierarchy_chart(frame: pd.DataFrame, context: str = "") -> go.Figure:
    ...
    figure = px.sunburst(frame, path=path, values=value, title=f"{value} Hierarchy: {' > '.join(path)}")
    return apply_theme(figure, source_context=context)
...
def scatter_chart(frame: pd.DataFrame, x: str = "Discount", y: str = "Profit", context: str = "") -> go.Figure:
    ...
    figure = px.scatter(data, x=x, y=y, opacity=0.55, title=f"{y} versus {x}")
...
def result_chart(frame: pd.DataFrame, spec: ChartSpec) -> go.Figure | None:
    ...
    if spec.chart_type == "line":
        ...
        figure = px.scatter(frame, x=spec.x, y=spec.y[0], title=spec.title)
