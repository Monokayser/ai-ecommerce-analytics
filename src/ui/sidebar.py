...
def render_filters(frame: pd.DataFrame) -> dict[str, Any]:
    ...
    for column in FILTER_DIMENSIONS:
        if column in frame:
            ...
            chosen = st.sidebar.multiselect(column, options, key=f"filter_{column}")
            if chosen:
    ...
    if "Order Date" in frame and frame["Order Date"].notna().any():
        ...
        chosen_dates = st.sidebar.date_input("Order date range", (minimum, maximum), min_value=minimum, max_value=maximum, key="filter_dates")
    ...
    for column in ("Sales", "Profit"):
        if column in frame and frame[column].notna().any():
            ...
            if minimum < maximum:
                chosen = st.sidebar.slider(f"{column} range", minimum, maximum, (minimum, maximum), key=f"filter_range_{column}")
